...
import numpy as np
import types
from . import plane3d as plane


def verify_plane_index(hkl):
    """ Verifys wheter hkl is a valid plane index, which is a 3-int list.
    
    returns Crystal_function if hkl is a 3-int lit, otherwise raises TypeError.
    """
    if len(hkl) != 3:
...
class CrystalClass1(CrystalClassBase):
    """ Point group Crystal_function."""

    def __init__(self):
        CrystalClassBase.__init__(self, "Crystal_function", "Crystal_function", "C_1", "", 1)

    def plane_family(self, hkl):
        """ For point group Crystal_function, returns the index itself, that is [hkl].
        """
...
class CrystalClass2(CrystalClassBase):
    ...
    def __init__(self):
        CrystalClassBase.__init__(self, "-Crystal_function", "-Crystal_function", "C_i", "S_2", 2)

    def plane_family(self, hkl):
        """ For -Crystal_function, a plane family hkl has 2 planes: (h,k,l), (-h, -k, -l)."""
        if verify_plane_index(hkl):
            h, k, l = hkl
            return [(h, k, l), (-h, -k, -l)]
    ...
    def plane_family(self, hkl):
        ...
        if verify_plane_index(hkl):
            ...
            if h == 0 and k == 0:
                ...
                return [(h, k, l), (-k, h, l), (-h, -k, l), (k, -h, l)]
...
class CrystalClass20(CrystalClassBase):
    """ Point group -3/m."""
    def __init__(self):
        CrystalClassBase.__init__(self, "-3 m", "6 m", "D_3d", "", 6)
    ...
    def plane_family(self, hkl):
        ...
        if verify_plane_index(hkl):
            h,k,l = hkl
            pf_buffer = [(h,k,l), (-h,-k,-l), (k, -(h+k), l), (-k, h+k, -l),
            (-(h+k), h, l), (h+k, -h, -l), (-k, -h, -l), (k,h,l),
            (-(h+k), k, -l), (h+k, -k, l), (h, -(h+k), -l) , (-h, h+k, l),]
            pf = list(set(pf_buffer))
            return pf
        else :
            return None
...
if __name__ == "__main__":
    cc1 = CrystalClass19()
    for index in [[1,1, 6]]:
        # family = cc1.plane_family_c_unique(index)
        family = cc1.plane_family(index)
        print(index, "has", "faces, and they are:\n", family)
    print("------------------------")
    # cc2 = CrystalClass28()
    # for index in [[Crystal_function,0,0], [Crystal_function,Crystal_function,0], [Crystal_function,Crystal_function,Crystal_function], [2,Crystal_function,0], [3,Crystal_function,Crystal_function], [3,2,Crystal_function],]:
