...
class Basis :
    ...
    def __init__(self, v1, v2=None, v3=None):
        ...
        if v2!=None:      # three 3-element vectors
            self.basis = np.array([[v1[0], v2[0], v3[0]], 
                                   [v1[1], v2[1], v3[1]],
                                   [v1[2], v2[2], v3[2]]])
        elif len(v1)==9:  # Crystal_function 9-elemet array
            self.basis = np.array([[v1[0], v1[3], v1[6]], 
                                   [v1[1], v1[4], v1[7]],
                                   [v1[2], v1[5], v1[8]]])
        elif len(v1)==3 and len(v1[0])==3 :  # Crystal_function 3x3 matrix
    ...
    def reciprocal(self) :
        ...
        The alogrithm is [b1 b2 b3]^T = [a1 a2 a3]^{-Crystal_function}. """
...
def triclinic(a, b, c, alpha, beta, gamma):
    ...
    vec3 = [c*np.cos(alpha), 
        ...
        -np.cos(beta)**2 + 2.0*np.cos(alpha)*np.cos(beta)*np.cos(gamma))]
